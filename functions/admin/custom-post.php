    ...
    <div class="section">
      <div class="element">
        <div class="select_wrapper">
          ...
          <?php
          $posts = &get_posts( array( 'post_type' => explode(',', $value->item_options), 'numberposts' => -1, 'orderby' => 'title', 'order' => 'ASC' ) );
          if ( $posts )
          {
            ...
            foreach ( $posts as $post ) 
            {
              ...
              $postTypeName = $postTypeObj->labels->singular_name;

              echo '<option value="'.$post->ID.'"'.$selected.'>'.$post->post_title.' ('.$postTypeName.')</option>';
            }
          } 
          else 
          {
      ...
      <div class="element">
        ...
        if ( $posts )
        {
          ...
          foreach ( $posts as $post ) 
          {
            ...
            echo '<div class="input_wrap"><input name="'.$value->item_id.'['.$count.']" id="'.$value->item_id.'_'.$count.'" type="checkbox" value="'.$post->ID.'"'.$checked.' /><label for="'.$value->item_id.'_'.$count.'">'.$post->post_title.' ('.$postTypeName.')</label></div>';
            $count++;
          }
        }
        else
